...
class BulkScanLauncher {
    ...
    private static ScanPool taskEngine = buildTaskEngine();
...
abstract class Scan implements IScannerCheck {
    ...
    void report(String title, String detail, Resp... requests) {
        ...
        log.append("Target: " + service.getProtocol() + "://" + service.getHost() + "\n");
        log.append("Issue: " + title + "\n");
        log.append(Utilities.helpers.bytesToString(reqs[0].getRequest()));
        log.append("\n--------------------------------------");
        SmuggleScannerTab.log(log.toString());
